...
export async function hmrcRoutes(app: FastifyInstance) {
  ...
  // ── Period summaries (quarterly MTD submission) ────────────────────
  //
  // This is the "minimum functionality" production review wants to see: the
  // user can actually submit a quarter to HMRC. All three are AMAP-basis —
  // buildPeriodSubmission folds the mileage allowance + AMAP-compatible
  // expenses and excludes motor running costs (fuel/insurance), per HMRC's
  // mutual-exclusion rule.

  const periodWindow = z.object({
    taxYear: z.string(),
    periodStartDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    periodEndDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  });

  // GET /hmrc/period-summary/preview — compute the quarter's figures from
  // MileClear data WITHOUT submitting. Local read only, no HMRC call. Drives
  // the review screen so the user sees exactly what will be sent.
  app.get("/period-summary/preview", { preHandler: authMiddleware }, async (request, reply) => {
    const parsed = periodWindow.safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.issues[0].message });
    }
    if (!isValidHmrcTaxYear(parsed.data.taxYear)) {
      return reply.status(400).send({ error: "Invalid tax year (expected YYYY-YY)" });
    }
    try {
      const payload = await buildPeriodSubmission({
        prisma,
        userId: request.userId!,
      ...
      return reply.send({
        data: { breakdown: payload.breakdown, periodDates: payload.periodDates },
      });
    } catch (err) {
      return reply
        .status(400)
        .send({ error: err instanceof Error ? err.message : "Could not build the period summary" });
    }
  });

  // POST /hmrc/period-summary — submit a quarter to HMRC. A duplicate/overlap
  // is surfaced cleanly (handleHmrcError) so the UI can route to amend.
  app.post("/period-summary", { preHandler: authMiddleware }, async (request, reply) => {
    const parsed = periodWindow.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.issues[0].message });
    }
    if (!isValidHmrcTaxYear(parsed.data.taxYear)) {
    ...
    if (!conn?.businessId) {
      return reply
        .status(400)
        .send({ error: "Confirm your self-employment business with HMRC before submitting." });
    }
    ...
    try {
      ...
      logEvent("hmrc.period_submitted", request.userId!, {
        ...
        turnoverPence: payload.breakdown.income.turnoverPence,
        mileageDeductionPence: payload.breakdown.mileage.deductionPence,
      });
      return reply.send({ data: { periodId: result.periodId, breakdown: payload.breakdown } });
    } catch (err) {
      return handleHmrcError(reply, err);
    }
  });

  // GET /hmrc/period-summary — list the quarters already submitted for the
  // user's business in a tax year (drives the "Q1 ✓ submitted" UI).
  app.get("/period-summary", { preHandler: authMiddleware }, async (request, reply) => {
    const parsed = z.object({ taxYear: z.string() }).safeParse(request.query);
    if (!parsed.success || !isValidHmrcTaxYear(parsed.data.taxYear)) {
      return reply.status(400).send({ error: "Invalid or missing tax year (expected YYYY-YY)" });
    }
    const ctx = await loadConnectionWithNino(request.userId!);
    if ("error" in ctx) return reply.status(ctx.status).send({ error: ctx.error });
    const conn = await prisma.hmrcConnection.findUnique({
    ...
    if (!conn?.businessId) {
      return reply.status(400).send({ error: "No HMRC business selected." });
    }
    try {
