...
export const RELEASE_NOTES: ReleaseNote[] = [
  {
    ...
    emailTagline:
      "Everything we've shipped over the last two weeks rolled into one big release. HMRC quarterly submissions, road-snapped routes, smarter trip detection that actually feels alive on your Lock Screen, the sole-trader features the community asked for, and an in-app help centre so you can find your way around without leaving the app.",
    emailHighlights: [
      ...
      "**Heartbeat alerts that catch silent failures.** If iOS turns off your background location, the app now pushes you a one-tap fix before you notice trips have stopped recording.",
      "**Session reliability rebuilt.** Refresh-token rotation now uses token families with replay detection — legitimate users no longer get spurious auto-logouts when iOS suspends the app mid-flight, while genuine token theft is detected and the whole session is force-terminated. End-Shift from the Lock Screen also properly closes the shift on the server instead of just dismissing the Live Activity, so today's drives no longer leak into yesterday's still-open shift.",
      "**No more phantom trips when you open the app.** Geofence Exit events fired in the first 30 seconds after app launch now demand a fresh high-accuracy GPS fix before we accept them — so iOS getting confused by a stale cell-tower fix at app start can no longer kick off a fake trip while you're sitting at your desk.",
      "**Trips with brief stops merge into one.** Drive 5 minutes, park for under a minute at a petrol station, drive another 10 — that's one trip in your list now, not two. We were rejecting the merge when the stop-detection split happened to overlap the new recording by a second, which is exactly when overlap is the strongest signal of a continuous drive.",
      "**Help & Tutorials right inside the app.** Avatar menu → Help & Tutorials. Categorised FAQ, replayable first-launch tour, and inline info buttons throughout the app — tap any ⓘ next to the Tax Readiness card, the PAYE field, the My Accountant row, or the tax basis toggle and a focused explainer slides up. No more guessing what something means.",
    ],
    items: [
